// One-off: render the new D6 editorial pin for an existing published article
// and post it directly to Pinterest so we can eyeball the design live.
//
// Usage: npx tsx scripts/post-d6-test.ts
...
import { generatePinImageEditorial } from "../src/lib/services/canva";
...
const EYEBROW = "SPRING RECIPE";
const SUBTITLE = "Zesty & Tart Bake";
...
async function main() {
  ...
  console.log("\nGenerating D6 editorial PNG...");
  const t0 = Date.now();
  const png = await generatePinImageEditorial({
  ...
  const blob = await put(`recipes/${article.slug}/pin-d6.png`, png, {
    access: "public",
    allowOverwrite: true,
  });
